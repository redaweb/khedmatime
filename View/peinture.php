<?php
session_start();
include '../model/get_services_peinture.php'; 
?>
    ...
    <div class="container mx-auto px-4">
        ...
        <div class="mb-10 text-center md:text-left">
            <h1 class="text-3xl font-extrabold text-gray-900 flex items-center justify-center md:justify-start">
                <i class="fas fa-paint-roller text-orange-500 mr-3"></i> Artisans en Peinture
            </h1>
            <p class="text-gray-500 mt-2">
                <span class="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-sm font-bold">
                    <?php echo count($services); ?> experts disponibles
                </span>
            </p>
        </div>
        ...
        <?php if(count($services) > 0): ?>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                <?php foreach($services as $artisan): ?>
                    <?php
                        // Gestion du lien : redirection vers login si non connecté
                        $link = isset($_SESSION['user_id']) ? "profil_artisan.php?id=" . $artisan['id_service'] : "../view/login.php";
                    ?>
        ...
        <?php else: ?>
            <div class="bg-white rounded-3xl p-20 text-center shadow-sm border border-gray-100">
                ...
                <h3 class="text-xl font-bold text-gray-800">Aucun peintre trouvé</h3>
                <p class="text-gray-500">Devenez le premier artisan peintre de votre région !</p>
                <a href="service.php" class="mt-6 inline-block bg-orange-500 text-white px-8 py-3 rounded-full font-bold transition hover:bg-orange-600">
